		...
		try{
			...
			for(String id:itemID)
			{
				...
				if(sqlCheckResults.next()){
					request.setAttribute("Error","Error occured: Cannot delete , client is tied to coaching session!");
				} else {
			...
			if(rs == 1){
				 request.getRequestDispatcher("/coachingAdmin.jsp").forward(request, response);
