...
/**
 * Servlet implementation class JdbcReadFile
 */
...
public class JdbcReadFile extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String query = "SELECT `banner_image`,`banner_image_type` FROM  `banner_image`  WHERE  `banner_image_id` = '"+id+"'";

			stmt = conn.createStatement();
			rs = stmt.executeQuery(query);
			if (rs.next()) {
				type=rs.getString("banner_image_type");
				photo = rs.getBlob(1);
